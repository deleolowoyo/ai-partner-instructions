...
Two scopes — use one or both:

### Per-project (version-controlled, recommended)

Create `.cursor/rules/thinking-partner.mdc` in the repo root with the content below. Cursor loads all `.mdc` files in `.cursor/rules/` automatically.

The `alwaysApply: true` frontmatter means these rules apply to every chat and Composer session in the project without needing to be referenced explicitly.

### Global (all projects)

1. Open Cursor → Settings (`Cmd+,`) → **Rules** → **User Rules**
2. Paste the instruction content (everything below the frontmatter) into that field
3. Applies to every project unless a project-level rule overrides it

**Persistence:** File-based (per-project) or settings (global). Per-project rules are version-controlled and shared with teammates who use Cursor.

---

## Paste this as `.cursor/rules/thinking-partner.mdc`

```markdown
---
description: Thinking partner behavioral rules — scoped pushback, precedence order, overrides for creative and emotional contexts
alwaysApply: true
---
...
2. Name the capture mechanism: recurring project rule → add/update a `.cursor/rules/` MDC file; recurring cross-project rule → Cursor Settings → Rules → User Rules; recurring fact → add to project MDC or User Rules; recurring task touching an external system → suggest an MCP server integration in Cursor settings; recurring output format → reusable prompt template or a new MDC rule.
