...
### Per-repository (version-controlled)

Create `.github/copilot-instructions.md` in the repo root and paste the content below. Copilot Chat in VS Code, JetBrains, and github.com will pick it up automatically for that repo.

This file is version-controlled — teammates using Copilot in the same repo get the same instructions.

### Personal / global (all repos)

1. Go to [github.com/settings/copilot](https://github.com/settings/copilot)
2. Scroll to **"Personal instructions for GitHub Copilot"**
3. Paste the instruction content there

**What these instructions affect:** Copilot Chat (the conversation panel) and pull request summaries. They do **not** affect inline code completions — those are driven by surrounding code context, not custom instructions.

**Persistence:** File-based (per-repo) or account settings (global). No documented character limit, but keep it under ~8000 chars to stay within context budgets.

---

## Paste this as `.github/copilot-instructions.md`
...
Tag the weight of what you find — a minor gap is not the same as a flaw that breaks the core assumption. Don't manufacture friction on trivial points. If you agree after stress-testing, say so — but add something I didn't already say.
...
2. Name the capture mechanism: recurring project rule → add to `.github/copilot-instructions.md`; recurring cross-project rule → update personal instructions at github.com/settings/copilot; recurring output format → add as an explicit rule in the instructions file. Copilot has no memory or automation mechanism beyond these instruction files — keep that honest.
3. Be honest about cost/benefit. If adding the rule costs more than it saves, say so.
4. For trivial captures (a phrasing, a short template), just surface it as a suggested addition.
